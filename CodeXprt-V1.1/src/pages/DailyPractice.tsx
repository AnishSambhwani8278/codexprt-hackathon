          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="prose dark:prose-invert max-w-none">
                ...
                <p className="text-gray-900 dark:text-gray-300">{selectedQuestion.description}</p>
                {selectedQuestion.hints && (
                  <>
                    <h3 className="text-lg font-semibold mt-4 mb-2 text-gray-900 dark:text-white">Hints</h3>
                    <ul>
                      {selectedQuestion.hints.map((hint, index) => (
                        <li key={index} className="text-gray-900 dark:text-gray-300">{hint}</li>
                      ))}
                    </ul>
                  </>
                )}
                {selectedQuestion.testCases && (
                  <>
                    <h3 className="text-lg font-semibold mt-4 mb-2 text-gray-900 dark:text-white">Test Cases</h3>
                    {selectedQuestion.testCases.map((testCase, index) => (
                      <div key={index} className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg mb-4">
                        <p className="text-gray-900 dark:text-gray-300"><strong>Input:</strong> {testCase.input}</p>
                        <p className="text-gray-900 dark:text-gray-300"><strong>Output:</strong> {testCase.output}</p>
                        <p className="text-gray-900 dark:text-gray-300"><strong>Explanation:</strong> {testCase.explanation}</p>
                      </div>
                    ))}
                  </>
                )}
